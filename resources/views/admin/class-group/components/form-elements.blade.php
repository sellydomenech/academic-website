...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('teacher_selected'), 'has-success': fields.teacher_selected && fields.teacher_selected.valid }">
    <label for="teacher_selected" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.class-group.columns.teacher_selected') }}</label>
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <multiselect v-model="form.teacher_selected" placeholder="{{ trans('brackets/admin-ui::admin.forms.select_options') }}" label="name" :options="{{ $teachers->toJson() }}" :multiple="false" :searchable="true"  track-by="id" open-direction="bottom"></multiselect>
        <div v-if="errors.has('teacher_selected')" class="form-control-feedback form-text" v-cloak>@{{ errors.first('teacher_selected') }}</div>
    </div>
</div>
